import * as React from 'react';

import { FieldShell, useFieldChrome } from '../lib/FieldShell';
import { DateTimePickerPrimitive } from './DateTimePickerPrimitive';
import type { FieldLayoutProps } from '../Input/Input.variants';
import type { RootProps } from '../lib';

export interface DateTimePickerProps extends RootProps, FieldLayoutProps {
  id?: string;
  name?: string;
  /** ISO 8601 datetime string (`YYYY-MM-DDTHH:MM`). */
  value?: string;
  onChange?: (next: string) => void;
  min?: Date;
  max?: Date;
  disabled?: boolean;
  required?: boolean;
  invalid?: boolean;
  readOnly?: boolean;
  label?: React.ReactNode;
  description?: React.ReactNode;
  error?: React.ReactNode;
  wrapperClassName?: string;
  labelClassName?: string;
  className?: string;
  'data-testid'?: string;
}

/** Chrome-wrapped Omni DateTimePicker. */
export const DateTimePicker = React.memo(
  React.forwardRef<HTMLDivElement, DateTimePickerProps>(
    (
      { id: idProp, wrapperClassName, labelClassName, layout = 'vertical', label, description, error, required, invalid, className, ...primitiveProps },
      ref,
    ) => {
      const { id, isInvalid, descriptionId, errorId, describedBy } = useFieldChrome({
        id: idProp,
        label,
        description,
        error,
        invalid,
        prefix: 'oui-datetime',
      });
      return (
        <FieldShell
          id={id}
          layout={layout}
          label={label}
          description={description}
          error={error}
          required={required}
          descriptionId={descriptionId}
          errorId={errorId}
          labelTag="span"
          wrapperClassName={wrapperClassName}
          labelClassName={labelClassName}
        >
          <DateTimePickerPrimitive
            ref={ref}
            id={id}
            invalid={isInvalid}
            required={required}
            aria-describedby={describedBy}
            className={className}
            {...primitiveProps}
          />
        </FieldShell>
      );
    },
  ),
) as React.NamedExoticComponent<DateTimePickerProps>;
(DateTimePicker as unknown as { displayName: string }).displayName = 'DateTimePicker';
